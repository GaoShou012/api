...
func (c *RbacMenuGroup) Create(ctx *gin.Context) {
	...
	operator := GetOperator(ctx)
	{
		...
		menuGroup := &params
		if err := global.TenantRBAC.CreateMenuGroup(operator,menuGroup); err != nil {
			libs_http.RspState(ctx, 1, err)
			return
		}
	}
...
func (c *RbacMenuGroup) Update(ctx *gin.Context){
	...
	operator := GetOperator(ctx)
	{
		...
		menuGroup := &params
		if err := global.TenantRBAC.UpdateMenuGroup(operator, menuGroupId, menuGroup); err != nil {
			libs_http.RspState(ctx,1,err)
			return
		}
	}
...
func (c *RbacMenuGroup) Delete(ctx *gin.Context) {
	...
	{
		id := params.Id
		ok, err := global.TenantRBAC.DeleteMenuGroup(operator, id)
		if err != nil {
			libs_http.RspState(ctx, 1, err)
			return
		}
