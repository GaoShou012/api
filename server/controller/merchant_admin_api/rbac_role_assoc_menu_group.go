...
func (c *RbacRoleAssocMenuGroup) Create(ctx *gin.Context){
	...
	if err := global.TenantRBAC.RoleAssocMenuGroup(operator,params.RoleId,params.MenuGroupId); err != nil {
		libs_http.RspState(ctx,1,err)
		return
	}
...
func (c *RbacRoleAssocMenuGroup) Delete(ctx *gin.Context) {
	...
	operator := GetOperator(ctx)
	if err := global.TenantRBAC.RoleDisassociateMenuGroup(operator,params.Id); err != nil{
		libs_http.RspState(ctx,1,err)
		return
	}
