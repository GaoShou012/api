...
func (p *plugin) GetById(topic string, messageId string) (stream.Event, error) {
	res, err := p.opts.redisClient.XRange(topic, messageId, messageId).Result()
	if err != nil {
		return nil, err
	}
	if len(res) == 0 {
...
func (p *plugin) Push(topic string, message []byte) (string, error) {
	...
	msgId, err := p.opts.redisClient.XAdd(xAddArgs).Result()
	return msgId, err
}
...
func (p *plugin) Pull(topic string, lastMessageId string, count uint64) ([]stream.Event, error) {
	...
	res, err := p.opts.redisClient.XRead(readArgs).Result()
	if err != nil {
		if err == redis.Nil {
			return nil, nil
		} else {
	...
	for _, row := range res {
		for _, message := range row.Messages {
			...
			if !ok {
				p.opts.redisClient.XDel(topic, message.ID)
...
func (p *plugin) Subscribe(topic string, handler stream.Handler) (stream.Subscriber, error) {
	wg := sync.WaitGroup{}
